...
description: Guidance for composing Codex and GPT-5.6 prompts for coding, review, diagnosis, and research tasks delegated from Claude Code. Prefer this over the Codex plugin's bundled gpt-5-4-prompting skill when targeting GPT-5.6 models such as gpt-5.6-sol.
user-invocable: false
---

# GPT-5.6 Prompting

Use this skill when delegating to Codex or another GPT-5.6-based workflow (e.g. `gpt-5.6-sol`) for help. Derived from the Codex plugin's `gpt-5-4-prompting` skill; maintained here because the plugin cache must not be edited locally.
...
Async review rules:
- Pin every review range by immutable commit hash (`<hash>~1..<hash>`), never `HEAD`-relative. Codex runs are queued and long-lived; the branch can move between prompt authoring and execution, silently changing what gets reviewed.
- Expect high-effort runs (`--effort xhigh`) to take 5–15+ minutes and outlive foreground timeout windows; plan for background retrieval of the result rather than treating a timeout as failure.
